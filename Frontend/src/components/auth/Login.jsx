...
import ConnectorGraphic from '../ui/ConnectorGraphic';
import loginHero from '../../../Reference4.png';
...
const Login = ({ onLogin, onNavigate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // No role is sent — the account's own role (User/Station Manager/Admin)
    // determines where they land after login. One email maps to exactly
    // one account, so there's nothing to disambiguate here.
  ...
  return (
    <div className="min-h-screen flex items-stretch">
      ...
      <div className="hidden lg:flex lg:w-[46%] flex-col justify-center px-16 py-12 relative overflow-hidden">
        <motion.img src={loginHero} alt="A ChargeWay charging station at night" aria-hidden="true"
          initial={{ scale: 1.08, opacity: 0 }} animate={{ scale: 1, opacity: 0.45 }} transition={{ duration: 1.2, ease: "easeOut" }}
          className="absolute inset-0 w-full h-full object-cover" />
        <div className="absolute inset-0"
          style={{ background: "linear-gradient(105deg, #0A0D14 35%, rgba(10,13,20,0.65) 65%, rgba(10,13,20,0.3) 100%)" }} />
        <motion.div className="absolute -top-24 -left-24 w-96 h-96 rounded-full opacity-20 blur-3xl pointer-events-none"
          animate={{ scale: [1, 1.15, 1], opacity: [0.15, 0.25, 0.15] }} transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
          style={{ background: "radial-gradient(circle, #5B47E0, transparent 70%)" }} />

        <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="relative max-w-md">
          <Logo size="lg" className="mb-8" />

          <h1 className="text-hero text-white leading-[1.05] mb-4">
            Plug in.<br />Power on.
          </h1>
            ...
            {BENEFITS.map((b, i) => (
              <motion.div key={b.title} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }}
                ...
                <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5"
                  style={{ background: "rgba(255,138,61,0.1)" }}>
                  <Icon name={b.icon} className="w-4.5 h-4.5 text-[#FF8A3D]" style={{ width: 18, height: 18 }} />
                </div>
                <div>
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
          ...
          <GlassCard className="p-8">
              ...
              <div>
                ...
                <div className="text-right mt-1.5">
                  <button type="button" onClick={() => setForgotOpen(true)}
                    className="text-xs text-[#FF8A3D] hover:text-[#FFAB70] font-medium">
                    Forgot password?
                  </button>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <GoogleAuthButton onAuthed={onLogin} onError={setError} />
            ...
            <div className="mt-5 pt-5 border-t border-white/5 text-center">
              <p className="text-slate-400 text-sm">
                ...
                <button onClick={() => onNavigate("register")} className="text-[#FF8A3D] font-semibold hover:text-[#FFAB70]">
                  Sign up
                </button>
              </p>
            </div>
