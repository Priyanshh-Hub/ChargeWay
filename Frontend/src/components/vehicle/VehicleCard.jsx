...
const VehicleCard = ({ vehicle, onEdit, onDelete, onFavorite, busy }) => {
  return (
    <motion.div layout initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }}>
      <GlassCard className={`p-0 overflow-hidden relative ${vehicle.isFavorite ? "ring-1" : ""}`} style={vehicle.isFavorite ? { boxShadow: "0 0 0 1px rgba(255,138,61,0.4)" } : {}}>
        ...
        <div className="p-4">
          ...
          <p className="font-mono text-[#FF8A3D] text-sm mt-2">{vehicle.vehicleNumber}</p>
          ...
          <div className="grid grid-cols-3 gap-2 mt-3 text-center">
            {[{ v: `${vehicle.range_km || 0} km`, l: "Range" }, { v: `${vehicle.battery_kwh || 0} kWh`, l: "Capacity" }, { v: vehicle.color || "—", l: "Color" }].map(item => (
              <div key={item.l} className="p-1.5 rounded-lg" style={{ background: "rgba(255,138,61,0.05)" }}>
                <p className="text-xs font-bold text-[#FF8A3D]">{item.v}</p>
          ...
          <div className="flex gap-2 mt-4">
            <button onClick={() => onEdit(vehicle)} disabled={busy}
              className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-medium border border-white/10 text-slate-300 hover:border-[#FF8A3D]/40 hover:text-[#FF8A3D] transition-all">
