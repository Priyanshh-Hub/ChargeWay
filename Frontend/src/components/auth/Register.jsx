...
const Register = ({ onLogin, onNavigate }) => {
  ...
  // Lets the landing page's "Partner With Us" section (and any other
  // marketing link) jump straight into the partner flow via
  // /register?role=partner instead of making someone click the toggle.
  const [step, setStep] = useState(1); // 1 personal, 2 station location, 3 station setup, 4 success
  const [role, setRole] = useState(searchParams.get("role") === "partner" ? "Station Manager" : "User");
  ...
  const [stationInfo, setStationInfo] = useState({ stationName: "", stationAddress: "", businessName: "", businessType: "", lat: "", lng: "" });
  ...
  // Shown to a prospective partner before they apply — "where is the
  // discussion of how much ChargeWay charges" was a fair question; this
  // answers it up front instead of leaving it undisclosed until later.
  const [commissionPct, setCommissionPct] = useState(null);

  useEffect(() => {
    if (role !== "Station Manager") return;
    api.get("/platform-config/partner-terms").then(res => {
      if (res.ok) setCommissionPct(res.data.partnerCommissionPct);
    });
  }, [role]);
  ...
  // "Partner of what?" — the venue actually hosting the charger. Answers
  // this explicitly for admin review instead of leaving it unasked.
  const businessTypes   = ["Independent / Standalone", "Shopping Mall", "Hotel / Resort", "Highway Rest Stop", "Residential Society", "Office / Corporate Park", "Fuel Station", "Other"];
  ...
  const handleFinalSubmit = async () => {
    ...
    const payload = {
      ...
      ...(role === "Station Manager" && {
        ...
        businessName: stationInfo.businessName, businessType: stationInfo.businessType,
            ...
            <React.Fragment key={num}>
              <div className="flex flex-col items-center gap-1">
                <div className="w-9 h-9 rounded-full flex items-center justify-center font-black text-sm transition-all duration-300"
                  style={{ background: isDone ? "linear-gradient(135deg,#10b981,#06b6d4)" : isActive ? "linear-gradient(135deg,#5B47E0,#FF8A3D)" : "rgba(255,255,255,0.08)", border: isActive ? "2px solid rgba(255,138,61,0.5)" : "2px solid transparent", color: isActive || isDone ? "white" : "#64748b", boxShadow: isActive ? "0 0 20px rgba(255,138,61,0.3)" : "none" }}>
                  {isDone ? "✓" : num}
                </div>
                <p className="text-xs font-medium hidden sm:block" style={{ color: isActive ? "#FF8A3D" : isDone ? "#10b981" : "#475569" }}>{label}</p>
              </div>
              {i < steps.length - 1 && (
      ...
      <div className="min-h-screen flex items-center justify-center p-4">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md text-center">
          <GlassCard className="p-8">
            ...
            {role === "Station Manager" && (
              <div className="p-3 rounded-xl text-left text-xs space-y-1.5 mb-4" style={{ background: "rgba(167,139,250,0.08)", border: "1px solid rgba(167,139,250,0.2)" }}>
                <p className="font-semibold flex items-center gap-1.5" style={{ color: "#a78bfa" }}>🤝 Partner application submitted</p>
                <p className="text-slate-400">
                  Your station is saved and offline for now — an Admin needs to review and approve it before
                  it appears to riders or accepts bookings. We'll notify you the moment there's a decision.
                </p>
              </div>
            )}

            <div className="p-3 rounded-xl text-left text-xs space-y-1.5 mb-6" style={{ background: "rgba(255,138,61,0.05)", border: "1px solid rgba(255,138,61,0.15)" }}>
              <p className="text-[#FF8A3D] font-semibold flex items-center gap-1.5"><Icon name="mail" className="w-3.5 h-3.5" /> Verify your email</p>
              ...
              {devVerifyUrl && (
                <p className="text-slate-500 pt-1">
                  Dev mode — no email server configured. <span className="text-[#FF8A3D]">Verification would open:</span> <code className="text-slate-400">{devVerifyUrl}</code>
                </p>
              )}
            </div>
          ...
          <div className="flex rounded-2xl p-1 mb-6" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}>
            {["User", "Station Manager"].map(r => (
              <button key={r} onClick={() => { setRole(r); setErrors({}); }}
                ...
                style={{ background: role === r ? "linear-gradient(135deg,#5B47E0,#FF8A3D)" : "transparent", color: role === r ? "white" : "#64748b" }}>
                {r === "User" ? "👤 User" : "🤝 Partner"}
              </button>
            ))}
          </div>
        ...
        <GlassCard className="p-8">
          ...
          <AnimatePresence mode="wait">
            {step === 1 && (
              <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                ...
                <Checkbox
                  ...
                  label={<>I agree to the <button type="button" onClick={(e) => { e.preventDefault(); setTermsOpen(true); }} className="text-[#FF8A3D] underline hover:text-[#FFAB70]">Terms & Conditions</button> and Privacy Policy</>}
                />
                ...
                <div className="grid grid-cols-2 gap-3">
                  <GoogleAuthButton onAuthed={onLogin} role={role} onError={setGeneralError} />
            ...
            {step === 2 && (
              <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                <div className="mb-4">
                  <h2 className="text-lg font-black text-white">Station Location</h2>
                  <p className="text-slate-400 text-xs mt-0.5">
                    This creates a partner application — an Admin reviews it before your station goes live on ChargeWay.
                  </p>
                </div>
                {commissionPct != null && (
                  <div className="p-3 rounded-xl text-xs" style={{ background: "rgba(124,106,232,0.08)", border: "1px solid rgba(124,106,232,0.2)" }}>
                    <p className="font-semibold mb-0.5" style={{ color: "#a78bfa" }}>💰 What ChargeWay charges</p>
                    <p className="text-slate-400">
                      ChargeWay takes <span className="text-white font-semibold">{Math.round(commissionPct * 100)}%</span> of
                      the energy revenue from bookings at your station — you keep the rest. (ChargeWay's separate platform
                      fee, paid by the rider on top of the energy cost, doesn't come out of your share.)
                    </p>
                  </div>
                )}
                ...
                <InputField label="Business / Owner Name" value={stationInfo.businessName} onChange={sp("businessName")} error={errors.businessName}
                  placeholder="e.g. Sunrise Mall Pvt Ltd (leave blank to reuse station name)" />
                <div>
                  <label className="block text-sm font-medium text-slate-300 mb-1.5">What kind of venue is this?</label>
                  <select value={stationInfo.businessType} onChange={sp("businessType")}
                    className="w-full rounded-xl px-4 py-2.5 text-white text-sm outline-none border"
                    style={{ background: "rgba(255,255,255,0.05)", borderColor: "rgba(255,255,255,0.1)" }}>
                    <option value="">— Select (helps us review your application) —</option>
                ...
                <div className="p-3 rounded-xl text-xs space-y-1" style={{ background: "rgba(255,138,61,0.05)", border: "1px solid rgba(255,138,61,0.15)" }}>
                  <p className="text-[#FF8A3D] font-semibold">📍 How to find Latitude & Longitude</p>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[
                      ...
                      <button key={c.city} onClick={() => setStationInfo(p => ({ ...p, lat: c.lat, lng: c.lng }))}
                        className="text-xs px-3 py-1.5 rounded-lg border border-white/10 text-slate-400 hover:border-[#FF8A3D]/40 hover:text-[#FF8A3D] transition-all"
              ...
              <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
                ...
                <div>
                  ...
                  <div className="relative">
                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[#FF8A3D] font-bold text-sm">₹</span>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button onClick={addCharger} className="text-xs px-3 py-1 rounded-lg font-medium text-[#FF8A3D] border border-[#FF8A3D]/30 hover:bg-[#FF8A3D]/10 transition-all">+ Add Charger</button>
                  </div>
                  ...
                  <p className="text-xs text-slate-500 mt-2">Total chargers: <span className="text-[#FF8A3D] font-bold">{stationDetails.chargers.reduce((s, c) => s + (parseInt(c.count) || 0), 0)}</span></p>
                </div>
                <div>
                    ...
                    {facilityOptions.map(f => {
                      ...
                      return (
                        <button key={f} onClick={() => toggleFacility(f)}
                          ...
                          style={{ background: selected ? "rgba(255,138,61,0.15)" : "rgba(255,255,255,0.03)", borderColor: selected ? "rgba(255,138,61,0.5)" : "rgba(255,255,255,0.1)", color: selected ? "#FF8A3D" : "#64748b" }}>
                          {selected ? "✓ " : ""}{f}
                        </button>
                      );
                    })}
          ...
          {step === 1 && (
            <p className="text-center text-slate-400 text-sm mt-5">
              Have an account?{" "}
              <button onClick={() => onNavigate("login")} className="text-[#FF8A3D] font-semibold hover:text-[#FFAB70]">Sign In</button>
            </p>
          )}
        </GlassCard>
      </motion.div>
