    ...
    <div className="space-y-6">
      ...
      <div className="flex items-end justify-between flex-wrap gap-4">
        ...
        {bookings.length > 0 && (
          <div className="flex gap-5 text-right">
            <div>
              <p className="text-2xl font-black text-[#FF8A3D] cw-figure">₹{totalSpend.toFixed(0)}</p>
              <p className="text-xs text-slate-500">Total Spent</p>
            </div>
            <div>
              <p className="text-2xl font-black text-green-400 cw-figure">{totalEnergy.toFixed(1)}</p>
              <p className="text-xs text-slate-500">kWh Used</p>
            </div>
            <div>
              <p className="text-2xl font-black text-emerald-400 cw-figure">{totalCarbon.toFixed(1)}</p>
      ...
      {bookings.length === 0 ? (
        ...
        <GlassCard className="p-16 text-center">
          ...
          <button onClick={clearFilters} className="text-[#FF8A3D] text-sm mt-2 hover:text-[#FFAB70]">Clear filters</button>
        </GlassCard>
      ) : (
          ...
          {visible.map(({ b, index }) => {
            ...
            return (
              <motion.div key={b._id}
                ...
                <GlassCard className="p-0 overflow-hidden hover:border-[#E8730F]/20 transition-all cursor-pointer"
                  onClick={() => setPreview({ b, index })}>
                  <div className="flex items-stretch">
                    {/* Accent bar */}
                    <div className="w-1 shrink-0" style={{ background: "linear-gradient(180deg,#5B47E0,#FF8A3D,#10b981)" }} />
                    ...
                    <div className="flex-1 p-5 flex items-center justify-between gap-4 flex-wrap">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0 relative"
                          style={{ background: "linear-gradient(135deg,#5B47E0,#FF8A3D)", boxShadow: "0 0 16px rgba(255,138,61,0.3)" }}>
                        ...
                        <div>
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            <span className="font-mono text-xs px-2 py-0.5 rounded-md font-bold"
                              style={{ background: "rgba(255,138,61,0.12)", color: "#FF8A3D", border: "1px solid rgba(255,138,61,0.25)" }}>
                              {serialNo}
                            </span>
                          </div>
                      ...
                      <div className="flex items-center gap-5 shrink-0">
                        ...
                        <div className="flex flex-col gap-2">
                          <button
                            ...
                            style={{ background: "rgba(255,138,61,0.1)", border: "1px solid rgba(255,138,61,0.3)", color: "#FF8A3D" }}>
                            Preview
                          </button>
                          <button
        ...
        {preview && (
          <motion.div className="fixed inset-0 z-50 flex items-center justify-center p-4"
            ...
            <motion.div
              ...
              style={{ background: "#070f1a", border: "1px solid rgba(255,138,61,0.2)", boxShadow: "0 0 60px rgba(91,71,224,0.3)" }}>
              <DarkInvoiceCard
...
const DarkInvoiceCard = ({ b, index, user, getSerial, carbonSaved, onClose, onDownload, onPrint, onShare }) => {
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', system-ui, sans-serif", color: "#e2e8f0", background: "#070f1a" }}>
      ...
      {/* Header */}
      <div style={{ position: "relative", padding: "28px 28px 24px", overflow: "hidden", borderBottom: "1px solid rgba(255,138,61,0.15)" }}>
        {/* BG glow */}
        <div style={{ position: "absolute", top: -40, right: -40, width: 180, height: 180, borderRadius: "50%", background: "radial-gradient(circle,rgba(91,71,224,0.25),transparent 70%)", pointerEvents: "none" }} />
        <div style={{ position: "absolute", bottom: -20, left: -20, width: 120, height: 120, borderRadius: "50%", background: "radial-gradient(circle,rgba(255,138,61,0.12),transparent 70%)", pointerEvents: "none" }} />

        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", position: "relative" }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
              <div style={{ width: 32, height: 32, borderRadius: 8, background: "linear-gradient(135deg,#5B47E0,#FF8A3D)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 0 12px rgba(255,138,61,0.4)" }}>
            ...
            <div style={{ display: "inline-block", padding: "2px 10px", borderRadius: 4, fontSize: 9, fontWeight: 800, letterSpacing: 2.5, background: "rgba(255,138,61,0.12)", border: "1px solid rgba(255,138,61,0.3)", color: "#FF8A3D" }}>
              TAX INVOICE
            </div>
          </div>
          <div style={{ textAlign: "right" }}>
            <div style={{ fontSize: 9, color: "#475569", letterSpacing: 1.5, marginBottom: 4 }}>INVOICE NUMBER</div>
            <div style={{ fontSize: 16, fontWeight: 900, fontFamily: "monospace", color: "#FF8A3D", letterSpacing: 1 }}>{serialNo}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, padding: "20px 28px", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
        ...
        <div style={{ padding: "14px", borderRadius: 10, background: "rgba(255,138,61,0.04)", border: "1px solid rgba(255,138,61,0.12)" }}>
          <div style={{ fontSize: 9, color: "#475569", letterSpacing: 1.5, fontWeight: 700, marginBottom: 8 }}>VEHICLE</div>
          <div style={{ fontWeight: 900, fontSize: 15, color: "#FF8A3D", fontFamily: "monospace", letterSpacing: 1 }}>{b.vehicleNumber}</div>
      ...
      <div style={{ padding: "20px 28px", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
        ...
        <Row label="Energy Dispensed"  value={`${b.energyKwh} kWh`}  highlight="#FF8A3D" />
      ...
      <div style={{ padding: "20px 28px 0" }}>
        ...
        {b.priceAdjustment < 0 && <Row label="UPI Prepaid Discount" value={`-₹${Math.abs(b.priceAdjustment).toFixed(2)}`} highlight="#22c55e" />}
        {b.priceAdjustment > 0 && <Row label="Cash Handling Surcharge" value={`₹${b.priceAdjustment.toFixed(2)}`} />}
        {b.lateFee > 0 && <Row label={`Late Arrival Fee (${b.minutesLate} min)`} value={`₹${b.lateFee}`} highlight="#f59e0b" />}
      ...
      {/* Total */}
      <div style={{ margin: "16px 28px", padding: "18px 20px", borderRadius: 12, background: "linear-gradient(135deg,rgba(91,71,224,0.2),rgba(255,138,61,0.1))", border: "1px solid rgba(255,138,61,0.3)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ fontSize: 34, fontWeight: 900, color: "#FF8A3D", textShadow: "0 0 20px rgba(255,138,61,0.4)" }}>₹{b.totalCost}</div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16, padding: "16px 28px", borderTop: "1px solid rgba(255,255,255,0.05)", background: "rgba(0,0,0,0.2)" }}>
        ...
        <div style={{ fontSize: 10, color: "#334155", lineHeight: 1.8 }}>
          <span style={{ color: "#FF8A3D", fontWeight: 700 }}>ChargeWay</span> · support@chargeway.com · www.chargeway.in<br />
      ...
      <div style={{ display: "flex", gap: 8, padding: "16px 28px 24px", flexWrap: "wrap" }}>
        <button onClick={onDownload}
          style={{ flex: "1 1 auto", background: "linear-gradient(135deg,#5B47E0,#FF8A3D)", color: "#fff", border: "none", borderRadius: 10, padding: 13, fontSize: 13, fontWeight: 800, cursor: "pointer", boxShadow: "0 4px 16px rgba(91,71,224,0.35)", letterSpacing: 0.3 }}>
          ↓ Download
        </button>
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .wrap{width:100%;max-width:600px;background:#070f1a;border-radius:20px;overflow:hidden;border:1px solid rgba(255,138,61,0.18);box-shadow:0 0 80px rgba(91,71,224,0.2)}
  .header{position:relative;padding:32px 36px 28px;border-bottom:1px solid rgba(255,138,61,0.12);overflow:hidden}
  .header::before{content:'';position:absolute;top:-50px;right:-50px;width:220px;height:220px;border-radius:50%;background:radial-gradient(circle,rgba(91,71,224,0.2),transparent 70%);pointer-events:none}
  .logo-row{display:flex;align-items:center;gap:10px;margin-bottom:10px}
  .logo-icon{width:36px;height:36px;border-radius:9px;background:linear-gradient(135deg,#5B47E0,#FF8A3D);display:flex;align-items:center;justify-content:center;font-size:18px;box-shadow:0 0 14px rgba(255,138,61,0.4)}
  .logo-text{font-size:22px;font-weight:900;color:#fff;letter-spacing:-0.5px}
  .tax-badge{display:inline-block;padding:2px 10px;border-radius:4px;font-size:9px;font-weight:800;letter-spacing:2.5px;background:rgba(255,138,61,0.12);border:1px solid rgba(255,138,61,0.3);color:#FF8A3D}
  .inv-meta{text-align:right}
  .inv-label{font-size:9px;color:#475569;letter-spacing:1.5px;font-weight:700;margin-bottom:4px}
  .inv-num{font-size:17px;font-weight:900;font-family:monospace;color:#FF8A3D;letter-spacing:1px}
  ...
  .info-box.cyan{background:rgba(255,138,61,0.04);border:1px solid rgba(255,138,61,0.14)}
  ...
  .vehicle-no{font-weight:900;font-size:16px;color:#FF8A3D;font-family:monospace;letter-spacing:1px}
  ...
  .total-box{margin:16px 36px;padding:18px 22px;border-radius:12px;background:linear-gradient(135deg,rgba(91,71,224,0.2),rgba(255,138,61,0.1));border:1px solid rgba(255,138,61,0.3);display:flex;justify-content:space-between;align-items:center}
  ...
  .total-amt{font-size:36px;font-weight:900;color:#FF8A3D;text-shadow:0 0 20px rgba(255,138,61,0.4)}
  ...
  .btn-primary{background:linear-gradient(135deg,#5B47E0,#FF8A3D);color:#fff}
...
<body><div class="wrap">
  ...
  <div class="section">
    ...
    ${b.priceAdjustment < 0 ? `<div class="row"><span class="row-label">UPI Prepaid Discount</span><span class="row-value" style="color:#22c55e">-₹${Math.abs(b.priceAdjustment).toFixed(2)}</span></div>` : ""}
    ${b.priceAdjustment > 0 ? `<div class="row"><span class="row-label">Cash Handling Surcharge</span><span class="row-value">₹${b.priceAdjustment.toFixed(2)}</span></div>` : ""}
    ${b.lateFee > 0 ? `<div class="row"><span class="row-label">Late Arrival Fee (${b.minutesLate} min)</span><span class="row-value" style="color:#f59e0b">₹${b.lateFee}</span></div>` : ""}
