  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex gap-3 flex-wrap">
          <div className="flex-1 min-w-48 relative">
            ...
            <input type="text" value={search} onChange={e => setSearch(e.target.value)}
              ...
              style={{ background: "rgba(255,255,255,0.05)", borderColor: search ? "rgba(255,138,61,0.4)" : "rgba(255,255,255,0.1)" }} />
          </div>
          ...
          <button onClick={() => setShowFilter(p => !p)}
            ...
            style={{ background: showFilter ? "rgba(255,138,61,0.1)" : "rgba(255,255,255,0.05)", borderColor: showFilter ? "rgba(255,138,61,0.4)" : "rgba(255,255,255,0.1)", color: showFilter ? "#FF8A3D" : "#94a3b8" }}>
            <Icon name="analytics" className="w-4 h-4" />
            Filters {activeFilterCount > 0 && <span className="w-5 h-5 rounded-full bg-[#FF8A3D] text-black text-xs font-black flex items-center justify-center">{activeFilterCount}</span>}
          </button>
      ...
      <p className="text-slate-400 text-sm">
        ...
        {search && <span> for "<span className="text-[#FF8A3D]">{search}</span>"</span>}
      </p>
      ...
      {filtered.length === 0 ? (
        <EmptyState icon="stations" title="No stations match your search"
          ...
          action={<button onClick={clearFilters} className="text-[#FF8A3D] text-sm hover:text-[#FFAB70]">Clear filters</button>} />
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {pageStations.map(s => {
              ...
              <div key={s._id}
                  ...
                  <div onClick={() => onViewStation(s)} className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full flex-shrink-0"
                        ...
                        title={s.networkType === "Partner" ? "Independently owned & operated — listed on ChargeWay" : "Owned and operated by ChargeWay"}>
                        {s.networkType === "Partner" ? "🤝 Partner" : "⚡ ChargeWay"}
                      </span>
                    </div>
                ...
                <div onClick={() => onViewStation(s)}>
                  <div className="flex items-center justify-between text-sm mb-2">
                    ...
                    <div className="flex items-center gap-2">
                      {s.peakPricing?.enabled && (
                        isPeakHour(new Date().getHours(), s.peakPricing)
                          ? <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full text-amber-400" style={{ background: "rgba(245,158,11,0.1)" }}>⚡ Peak now</span>
                          : <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full text-green-400" style={{ background: "rgba(34,197,94,0.1)" }}>🌙 Off-peak now</span>
                      )}
                      <span className="text-[#FF8A3D] font-bold cw-figure">₹{s.price_per_kwh}/kWh</span>
                    </div>
                  </div>
                ...
                <div className="flex gap-2 mt-3">
                  <button onClick={() => onViewStation(s)}
                    className="flex-1 text-center py-2 rounded-xl text-xs font-bold text-[#FF8A3D] border border-[#FF8A3D]/20 transition-all hover:bg-[#FF8A3D]/10"
                  ...
                  <button
                    ...
                    style={{ background: "linear-gradient(135deg,#5B47E0,#FF8A3D)", color: "#fff", border: "none", cursor: "pointer", boxShadow: "0 0 10px rgba(91,71,224,0.25)" }}>
                    🗺️ Navigate
                  </button>
                </div>
              </div>
