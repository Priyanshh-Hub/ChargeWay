...
const QRModal = ({ booking, station, onClose }) => (
  <Modal open onClose={onClose} title="Check-in PIN">
    <div className="flex flex-col items-center gap-2 p-6 rounded-2xl"
      style={{ background: "linear-gradient(135deg, rgba(91,71,224,0.12), rgba(255,138,61,0.08))", border: "1px solid rgba(255,138,61,0.2)" }}>
      <p className="text-slate-400 text-xs uppercase tracking-wider font-semibold">Give this code to station staff</p>
      <p className="text-5xl font-black text-[#FF8A3D] cw-figure tracking-[0.25em]">{booking.checkInOtp || "----"}</p>
      <p className="text-slate-500 text-xs text-center mt-1">Charger #{booking.chargerId} · {booking.stationName} · {booking.timeSlot}</p>
    </div>
    {booking.checkedIn && (
      <p className="text-green-400 text-xs text-center mt-3 font-semibold">✓ Already verified — charging in progress</p>
    )}
  </Modal>
);
...
const BookingsPage = ({ user }) => {
  ...
  const filtered = bookings
    ...
    .filter(b => !search.trim() || (b.stationName || b.stationId?.name || "").toLowerCase().includes(search.trim().toLowerCase()));
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex gap-2 flex-wrap items-center">
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search station..."
              className="pl-8 pr-3 py-1.5 rounded-lg text-xs bg-white/5 border border-white/10 text-white placeholder:text-slate-500 focus:outline-none focus:border-[#FF8A3D]/50 w-36 sm:w-44" />
          </div>
          {["All", "Upcoming", "Completed", "Cancelled"].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${filter === f ? "border-[#FF8A3D]/50 text-[#FF8A3D] bg-[#FF8A3D]/10" : "border-white/10 text-slate-400 hover:border-white/20"}`}>
              {f}
            </button>
          ))}
      ...
      {loading ? (
        ...
        <EmptyState icon="booking"
          title={search ? "No matching bookings" : filter === "All" ? "No bookings yet" : `No ${filter.toLowerCase()} bookings`}
          subtitle={search ? "Try a different station name." : "Find a station and reserve a charger to see it here."} />
      ) : (
            ...
            return (
              <GlassCard key={b._id} className="p-5">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <p className="text-[#FF8A3D] font-black cw-figure">₹{b.totalCost}</p>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
                  {[
                    ...
                    <div key={item.l} className="p-2 rounded-lg text-xs" style={{ background: "rgba(255,255,255,0.03)" }}>
                      ...
                      <p className="text-white font-semibold mt-0.5 cw-figure">{item.v}</p>
                    </div>
                  ))}
                </div>
                    ...
                    {b.status === "Upcoming" && (
                      <>
                        <Btn variant="outline" onClick={() => setQrBooking(b)} className="text-xs py-1.5 px-3 gap-1">
                          <Icon name="search" className="w-3.5 h-3.5 inline mr-1" />Check-in PIN
                        </Btn>
