        ...
        {phase === "loading" && (
          <motion.div key="loader"
            ...
            <div className="relative w-24 h-24">
              ...
              <div className="absolute inset-3 rounded-full border border-[#FF8A3D]/20 animate-pulse"
            ...
            <div className="w-48 h-1 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.08)" }}>
              <motion.div className="h-full rounded-full"
                style={{ background: "linear-gradient(90deg,#5B47E0,#FF8A3D)" }}
        ...
        {phase === "welcome" && (
          <motion.div key="welcome"
            ...
            {user?.role === "User" && (
              <>
                ...
                <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }}>
                  ...
                  <p className="text-[#FF8A3D] mt-2 font-semibold">Ready to Charge ⚡</p>
            ...
            {user?.role === "Station Manager" && (
              <>
                <div className="relative w-28 h-28 mx-auto">
                  {[0, 1, 2].map(i => (
                    <motion.div key={i} className="absolute inset-0 rounded-full border-2 border-[#FF8A3D]"
                      animate={{ scale: [1, 2.5], opacity: [0.8, 0] }}
                      transition={{ delay: i * 0.4, duration: 1.5, repeat: Infinity }} />
                  ))}
                  <div className="absolute inset-0 flex items-center justify-center">
                    <Icon name="bolt" className="w-14 h-14 text-[#FF8A3D]" />
                  </div>
                </div>
                <div>
                  <p className="text-slate-400 text-sm">Welcome back</p>
                  <h1 className="text-3xl font-black mt-1">{user.name.split(" ")[0]}</h1>
                  <p className="text-[#FF8A3D] mt-1">Station Control Active</p>
                </div>
              </>
            )}
            ...
            {user?.role === "Admin" && (
              <>
                ...
                <div>
                  ...
                  <p className="text-[#FF8A3D] text-sm mt-1">System Overview</p>
                </div>
                <div className="grid grid-cols-2 gap-4 max-w-xs mx-auto">
                  {[
                    ...
                    <div key={item.l} className="p-4 rounded-2xl border border-white/10" style={{ background: "rgba(255,138,61,0.05)" }}>
                      <p className="text-xs text-slate-400">{item.l}</p>
                      <p className="text-2xl font-black text-[#FF8A3D]"><AnimatedNumber to={item.v} /></p>
                    </div>
                  ))}
                </div>
              </>
            ...
            <motion.div className="w-40 h-1 mx-auto rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.08)" }}>
              <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg,#5B47E0,#FF8A3D)" }}
