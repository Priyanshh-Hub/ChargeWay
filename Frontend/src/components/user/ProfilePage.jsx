...
const ProfilePage = ({ user, onUserUpdate, setActiveView, onLogout }) => {
  ...
  const roleColor = {
    'User':            { bg: 'rgba(255,138,61,0.15)',   border: 'rgba(255,138,61,0.4)',   text: '#FF8A3D'  },
  ...
  }[user.role] || { bg: 'rgba(255,138,61,0.15)', border: 'rgba(255,138,61,0.4)', text: '#FF8A3D' };
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <h2 className="text-page text-white">My Profile</h2>
      ...
      <GlassCard className="p-6">
        <div className="flex items-center gap-5">
          ...
          <div className="flex-1">
            <h3 className="text-card text-white">{user.name}</h3>
          ...
          <button onClick={() => { setEditMode(!editMode); setSuccess(''); setError(''); }}
            ...
            style={{ background: editMode ? 'rgba(239,68,68,0.1)' : 'rgba(255,138,61,0.1)', borderColor: editMode ? 'rgba(239,68,68,0.3)' : 'rgba(255,138,61,0.3)', color: editMode ? '#f87171' : '#FF8A3D' }}>
          ...
          <div>
            ...
            {editMode ? (
              <input value={name} onChange={e => setName(e.target.value)}
                ...
                style={{ background: 'rgba(255,255,255,0.05)', borderColor: 'rgba(255,138,61,0.4)' }} />
            ) : (
      ...
      {user.role === "User" && setActiveView && (
        <GlassCard className="p-6 flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl flex items-center justify-center" style={{ background: "rgba(255,138,61,0.1)" }}>
              <Icon name="car" className="w-5 h-5 text-[#FF8A3D]" />
            </div>
            <div>
          ...
          {[
            ...
            <div key={p.key} className="flex items-center justify-between p-3 rounded-xl" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
              ...
              <button onClick={() => togglePref(p.key)} disabled={prefsLoading}
                ...
                style={{ background: prefs[p.key] ? "linear-gradient(135deg,#5B47E0,#FF8A3D)" : "rgba(255,255,255,0.1)" }}
