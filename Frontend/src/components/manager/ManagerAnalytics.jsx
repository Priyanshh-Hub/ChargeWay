...
const ManagerAnalytics = ({ user }) => {
  const [stations, setStations] = useState([]); // ALL stations assigned to this manager
  ...
  useEffect(() => {
    (async () => {
      // Find ALL stations assigned to this manager — a manager can run more than one.
  ...
  // ── Compute stats from the SELECTED station's bookings only ──
  const stationBookings = bookings.filter(b => b.stationId === station._id || b.stationId?._id === station._id);
  const completed   = stationBookings.filter(b => b.status === "Completed");
  const upcoming    = stationBookings.filter(b => b.status === "Upcoming");
  const cancelled   = stationBookings.filter(b => b.status === "Cancelled");
  ...
  const recentBookings     = [...stationBookings].sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0)).slice(0, 5);
  const statusColors       = { Available: "#10b981", Charging: "#7C6AE8", Maintenance: "#fbbf24" };
  const bookingStatusColor = { Completed: "#10b981", Upcoming: "#7C6AE8", Cancelled: "#ef4444" };
    ...
    <div className="space-y-8">
      ...
      {stations.length > 1 && (
        <div className="flex items-center gap-3 flex-wrap">
          <p className="text-slate-500 text-sm font-medium">Analytics for:</p>
          {stations.map((s, i) => (
      ...
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-page text-white">{station.name}</h2>
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className="text-xs text-slate-500 cw-figure">₹{station.price_per_kwh}/kWh · {station.chargers?.length || 0} chargers</span>
          </div>
        </div>
        <p className="text-xs text-slate-600 italic">{stations.length > 1 ? `Station ${activeIdx + 1} of ${stations.length}` : "My Station Only"}</p>
      </div>

      {/* Stat Cards — all from THIS station only */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="My Revenue"       value={`₹${revenue.toFixed(0)}`}         icon="bolt"     color="#FF8A3D" />
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: "Upcoming",  count: upcoming.length,  color: "#7C6AE8" },
          ...
          {months.map((m, i) => (
            <div key={m} className="flex-1 flex flex-col items-center gap-2">
              ...
              <div className="w-full rounded-t-xl transition-all duration-700 relative overflow-hidden"
                style={{ height: `${Math.max(8, (monthlyRev[i] / maxRev) * 120)}px`, background: "linear-gradient(180deg,#5B47E0,#FF8A3D)", opacity: monthlyRev[i] === 0 ? 0.2 : 1 }}>
          ...
          {chargerStats.map(c => (
            <div key={c.id} className="flex items-center gap-4 p-4 rounded-xl border border-white/10"
              ...
              <div className="w-10 h-10 rounded-xl flex items-center justify-center font-black text-sm flex-shrink-0"
                style={{ background: "linear-gradient(135deg,#5B47E0,#FF8A3D)" }}>#{c.id}</div>
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.08)" }}>
                    <div className="h-full rounded-full"
                      style={{ width: `${completed.length > 0 ? (c.sessions / completed.length) * 100 : 0}%`, background: "linear-gradient(90deg,#5B47E0,#FF8A3D)" }} />
                  </div>
            ...
            {recentBookings.map(b => (
              <div key={b._id} className="flex items-center gap-4 p-3 rounded-xl border border-white/10"
                ...
                <div className="text-right">
                  <p className="text-[#FF8A3D] font-bold text-sm">₹{b.totalCost}</p>
        ...
        <GlassCard className="p-6">
          ...
          <div className="flex flex-wrap gap-2">
            {station.facilities.map(f => (
              <span key={f} className="text-sm px-3 py-1.5 rounded-full border border-[#FF8A3D]/20 text-[#FF8A3D]"
