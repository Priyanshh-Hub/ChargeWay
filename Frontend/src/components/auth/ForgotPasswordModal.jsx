    ...
    <Modal open={open} onClose={onClose} title={
      ...
      {step === "sent" && (
        <div className="space-y-4 mt-1">
          ...
          {isDev && devUrl && (
            <div className="p-3 rounded-xl text-xs" style={{ background: "rgba(255,138,61,0.05)", border: "1px solid rgba(255,138,61,0.15)" }}>
              <p className="text-[#FF8A3D] font-semibold mb-1">Dev mode — no email server configured</p>
              <p className="text-slate-400">Click below to simulate opening the reset link from your inbox.</p>
              <button type="button" onClick={() => setStep("reset")} className="text-[#FF8A3D] underline mt-2 font-medium">
