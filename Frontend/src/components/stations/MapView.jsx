...
const MapView = ({ stations, onSelectStation }) => {
  ...
  useEffect(() => {
    ...
    validStations.forEach(s => {
      ...
      marker.bindPopup(`
        <div style="background:#0F1928;color:white;padding:12px 16px;border-radius:12px;border:1px solid rgba(255,138,61,0.3);min-width:200px">
          <b style="color:#FF8A3D;font-size:14px">${s.name}</b>
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
            ...
            <b style="color:#FF8A3D">₹${s.price_per_kwh}/kWh</b>
          </div>
          <div id="goto-${s._id}" style="background:linear-gradient(135deg,#5B47E0,#FF8A3D);color:white;text-align:center;padding:7px;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer">
            View Station →
          </div>
        </div>
