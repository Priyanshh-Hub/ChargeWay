  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h2 className="text-page text-white">Analytics</h2>
        {isAdmin && (
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Revenue"  value={`₹${((totals?.revenue || 0) / 1000).toFixed(1)}K`} icon="bolt"     color="#FF8A3D" />
        <StatCard label="Total Users"    value={totals?.users    || 0}                              icon="users"    color="#7C6AE8" />
      ...
      <div className="rounded-2xl p-6 border border-green-400/20"
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: "Total Energy Delivered", value: `${totalEnergy.toFixed ? Number(totalEnergy).toFixed(1) : totalEnergy} kWh`, color: "#FF8A3D" },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <GlassCard className="p-6 lg:col-span-2">
          ...
          <Sparkline data={monthlyRevenue} color="#FF8A3D" height={70} />
          <BarChart data={monthlyRevenue.map(v => v / 100)} labels={months} color="#5B47E0" />
        </GlassCard>
            ...
            {Object.entries(stationRevenue || {}).map(([name, rev]) => (
              <div key={name}>
                <div className="flex justify-between text-xs mb-1">
                  ...
                  <span className="text-[#FF8A3D] font-bold">₹{rev.toFixed(0)}</span>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <GlassCard className="p-6">
          ...
          <Sparkline data={monthlyUsers} color="#7C6AE8" height={80} />
