    ...
    <div className={`relative flex-shrink-0 ${className}`} style={{ width: s.box, height: s.box }}>
      {glow && (
        <div className="absolute inset-0 rounded-2xl blur-md opacity-50"
          style={{ background: "linear-gradient(135deg, #FF8A3D, #5B47E0)" }} aria-hidden="true" />
      )}
    ...
    <div className={`flex items-center gap-2 ${className}`}>
      ...
      {showWordmark && (
        <span className={`${s.text} font-display font-bold tracking-tight`}
          style={{ background: "linear-gradient(90deg, #FF8A3D, #7C6AE8)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
          ChargeWay
        </span>
      )}
    </div>
