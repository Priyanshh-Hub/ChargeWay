...
export const Btn = ({ children, onClick, disabled, className = "", variant = "primary", type = "button", loading = false, ...rest }) => {
  const styles = {
    primary: { background: disabled || loading ? "rgba(100,100,100,0.3)" : "linear-gradient(135deg, #5B47E0, #FF8A3D)", color: "white" },
    ...
    outline: { background: "rgba(255,138,61,0.05)", border: "1px solid rgba(255,138,61,0.3)", color: "#FF8A3D" },
...
export const Spinner = ({ text = "Loading..." }) => (
  <div className="flex flex-col items-center justify-center py-16 gap-5">
    <div className="relative w-16 h-16">
      ...
      <div className="absolute inset-0 rounded-full border-2 border-transparent border-t-[#FF8A3D] border-r-[#5B47E0] animate-spin" />
      <div className="absolute inset-2 rounded-full border border-[#FF8A3D]/20 animate-pulse" />
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="text-[#FF8A3D] text-lg font-black">⚡</span>
      </div>
    </div>
...
export const Alert = ({ type = "error", message }) => {
  ...
  const cfg = {
    ...
    info:    { cls: "bg-[#E8730F]/10 border-[#E8730F]/20 text-[#FF8A3D]",   icon: "info" },
  ...
  <div>
    <label htmlFor={id} className="flex items-start gap-2.5 cursor-pointer select-none group">
      <span
        ...
        style={{
          width: 18, height: 18,
          background: checked ? "linear-gradient(135deg,#5B47E0,#FF8A3D)" : "rgba(255,255,255,0.05)",
    ...
    <div>
      ...
      <div className="relative">
        ...
        <button
          ...
          className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-[#FF8A3D] transition-colors"
...
export const EmptyState = ({ icon = "info", title, subtitle, action }) => (
  <div className="flex flex-col items-center justify-center text-center py-14 px-6">
    <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-4" style={{ background: "rgba(255,138,61,0.08)" }}>
      <Icon name={icon} className="w-7 h-7 text-[#FF8A3D]" />
    </div>
