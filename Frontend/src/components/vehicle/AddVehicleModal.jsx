...
const AddVehicleModal = ({ open, onClose, onAdded }) => {
  ...
  const submit = async () => {
    ...
    // No fake `battery` (current charge %) — see CarSelection.jsx for why.
    const res = await api.post("/user/vehicles", { ...selectedCar, nickname: nickname.trim(), vehicleNumber, batteryHealth: 100 });
    setLoading(false);
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Add a Vehicle" maxWidth="max-w-2xl">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-3 max-h-80 overflow-y-auto pr-1">
            {filteredBrands.map(b => (
              <motion.button key={b.name} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                ...
                className="cursor-pointer rounded-xl p-4 border border-white/10 flex flex-col items-center justify-center gap-2 h-24 hover:border-[#FF8A3D]/50 transition-all"
      ...
      {phase === "model" && (
        <div>
          <button onClick={() => setPhase("brand")} className="text-[#FF8A3D] text-sm mb-3 hover:text-[#FFAB70]">← Back to Brands</button>
          <div className="relative mb-4">
            <Icon name="search" className="w-4 h-4 text-slate-500 absolute left-4 top-1/2 -translate-y-1/2" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-80 overflow-y-auto pr-1">
            {filteredModels.map(car => (
              <button key={car.model} onClick={() => { setSelectedCar(car); setPhase("details"); }}
                className="text-left rounded-xl p-3 border border-white/10 hover:border-[#FF8A3D]/50 transition-all flex gap-3"
                style={{ background: "rgba(15,25,45,0.8)" }}>
      ...
      {phase === "details" && selectedCar && (
        <div className="space-y-4">
          <button onClick={() => setPhase("model")} className="text-[#FF8A3D] text-sm hover:text-[#FFAB70]">← Back to Models</button>
          <div className="flex items-center gap-3 p-3 rounded-xl" style={{ background: "rgba(255,138,61,0.05)" }}>
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Connector: <span className="text-[#FF8A3D]">{selectedCar.connectorType}</span></p>
            </div>
          </div>
          <Alert message={error} />
